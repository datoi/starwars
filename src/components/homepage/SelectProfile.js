...
const SelectProfile = () => {
    ...
    const handleVariantClick = async (variant, background, image, story) => {
    ...
    const selectClick = () => {
        localStorage.setItem('name', selectedVariantName)

        navigate("/todoitem");
    };
    const bodyStyle = {
        backgroundImage: selectedVariant ? `url(${process.env.PUBLIC_URL}/${selectedVariantBackground})` : 'none',
    ...
    const variants = [
        ...
        },
        {
            variant: 'people/10/',
            variantName: 'Obi-Wan',
            background: 'ObiWanBackground.jpg',
            image: 'ObiWan.png',
